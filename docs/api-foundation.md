...
Remaining API work includes workload/node/service resources, strategic merge patch/server-side apply, status and other subresources, OpenAPI, admission and full field validation/defaulting, immutable-object semantics, complete RBAC escalation/bind rules, node authorization, finalizers/namespace deletion, and Helm/client workflow coverage. Invalid selectors, unsupported field selectors, dry-run, unsupported patch media types, and unimplemented routes return errors; the existing tests do not certify the missing behavior.
