...
Remaining API work includes server-side apply, scale and streaming/eviction subresources, OpenAPI, the remaining admission chain and full field validation/defaulting, additional mutable Pod transitions, complete RBAC escalation/bind rules, remaining node-admission coverage, finalizers/namespace deletion, and Helm/client workflow coverage. Invalid selectors, unsupported field selectors, dry-run, unsupported patch media types, and unimplemented routes return errors; the existing tests do not certify the missing behavior.

The tower lab checkpoint runs as the project guest user's transient `h3s-api-foundation` systemd unit. The binary is `$H3S_ROOT/h3s/target/debug/h3s`; data and credentials are under `$H3S_ROOT/runtime` (default guest root `/var/lib/hedronetes`). Restart/stop only this named project unit. A declarative NixOS service module and production runtime installation remain required for M1.
...
PATCH accepts RFC 6902 `application/json-patch+json` and RFC 7396 `application/merge-patch+json`. The result follows the same validation and CAS write path as PUT, with a distinct RBAC `patch` permission; non-admin RBAC patches cannot bypass the temporary escalation guard. Failed tests, stale resourceVersion conditions, invalid values and oversized growth make no storage change. JSON patches are limited to 256 operations and each intermediate object is bounded to 2 MiB. Concurrent writes may return 409; callers should fetch current state and retry deliberately. Strategic merge uses pinned Kubernetes field strategies and follows this same write path; see [its behavior and verification](strategic-patches.md). Server-side apply still returns 415 pending field-ownership implementation.
