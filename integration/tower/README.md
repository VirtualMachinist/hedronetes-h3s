...
These guests are isolated under `LIMA_HOME=/Users/abdul-qadir/.local/share/hedronetes-m1/lima`. Never operate the protected Colima `builder` profile or its assets. Do not run global cleanup or change the host's default Lima/Colima configuration.
...
Provision on Tower with `/opt/homebrew/bin` in PATH and the explicit project LIMA_HOME, using `limactl validate` then `limactl start --yes --name=hedronetes-server nixos-server.yaml` and the corresponding worker command. Keep configuration and command logs in the project evidence directory. Never invoke an implicit/default instance.
